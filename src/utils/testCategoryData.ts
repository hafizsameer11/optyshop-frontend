    ...
    try {
        ...
        if (categoriesData.success && categoriesData.data?.categories) {
            ...
            if (categoriesData.data.categories.length > 0) {
                ...
                const categoryResponse = await fetch(`/api/categories/slug/${firstCategory.slug}`);
                ...
                if (categoryData.data?.category) {
                    ...
                    const subcategoriesResponse = await fetch(`/api/subcategories/by-category/${categoryData.data.category.id}`);
                    ...
                    const productsResponse = await fetch(`/api/products?category=${firstCategory.slug}&limit=5`);
...
window.testSpecificCategory = async function(categorySlug = 'contact-lenses') {
    ...
    try {
        ...
        const productsResponse = await fetch(`/api/products?category=${categorySlug}&limit=12`);
        const productsData = await productsResponse.json();
